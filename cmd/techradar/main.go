...
import (
	...
	"techradar_backend/internal/api/down_vote_technology_handler"
	"techradar_backend/internal/api/get_technologies_handler"
	"techradar_backend/internal/api/get_technology_handler"
	"techradar_backend/internal/api/up_vote_technology_handler"
...
func main() {
	...
	poolUseCase := pool_item.New(storage, storage)
	...
	router.Route("/api/vote", func(r chi.Router) {
		r.Post("/up", up_vote_technology_handler.New(poolUseCase).Handle)
		r.Post("/down", down_vote_technology_handler.New(poolUseCase).Handle)
	})
